...
-- ^\/customer\/(\w*)|2:x-authenticated-userid,^\/user\/batatinhas\/(\w*)|3:x-authenticated-userid
-- http://api-customer.siworks.cloudesk.com.br:8000/customer/12312 -> http://api-customer.siworks.cloudesk.com.br:8000/customer/225
-- http://api-customer.siworks.cloudesk.com.br:8000/user/batatinhas/12312 -> http://api-customer.siworks.cloudesk.com.br:8000/user/batatinhas/225

function transform_uri(conf)
  local var_sub = {}     
  local uriParts = splitRole(ngx.var.uri,"/") 
  local tempUri = "/"
  local deuMatch = false
  for rep, replaces in ipairs(conf.replace.uri) do
